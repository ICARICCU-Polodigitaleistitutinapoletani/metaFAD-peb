...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function (){
	  ...
	  window.onmessage = function(e){
	    ...
	    switch(event.type) {
	      case 'loadMap':
	        loadingModule.loadJsonOntology(event.data, event.ontologyName);
	        break;
	    }
	  };
	  window.document.addEventListener('loadMap', handleLoadMapEvent, false);
	  function handleLoadMapEvent(e) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function ( graph ){
	  ...
	  filterMenu.setup = function ( datatypeFilter, objectPropertyFilter, subclassFilter, disjointFilter, setOperatorFilter, nodeDegreeFilter ){
	    ...
	    addFilterItem(datatypeFilter, "datatype", "Proprietà", "#datatypeFilteringOption");
	    addFilterItem(objectPropertyFilter, "objectProperty", "Relazioni", "#objectPropertyFilteringOption");
	    addFilterItem(subclassFilter, "subclass", "Sottoclassi solitarie", "#subclassFilteringOption");
	    addFilterItem(disjointFilter, "disjoint", "Classi disgiunte", "#disjointFilteringOption");
	    addFilterItem(setOperatorFilter, "setoperator", "Operatori di insiemi", "#setOperatorFilteringOption");
...
/***/ (function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function(d3) {module.exports = function ( graph ){
	  ...
	  var DEFAULT_JSON_NAME = "cultural-ON"; // This file is loaded by default
	  ...
	  loadingModule.loadJsonOntology = function(json, name) {
	    graph.clearAllGraphData();
	    loadingModule.initializeLoader(false);
	    var urlString = String(location);
	    var parameterArray = identifyParameter(urlString);
	    loadGraphOptions(parameterArray); // identifies and loads configuration values
	    parseOntologyContent(JSON.stringify(json), name);
	  }
	...
	//2] File Upload to OWL2VOWL Converter
	  ...
	  /** -- PARSE JSON CONTENT -- **/
	  function parseOntologyContent( content, name ){
	    if(!name) {
	      name = "noAlternativeNameYet";
	    }
	    ontologyMenu.append_bulletPoint("Reading ontology graph ... ");
	    var _loader = ontologyMenu.getLoadingFunction();
	    _loader(content, ontologyIdentifierFromURL, name);
	  }
	  ...
	  function loadGraphOptions( parameterArray ){
	    ...
	    function loadDefaultConfig(){
	      var defaultConfig = graph.options().defaultConfig();
	      defaultConfig.filter_disjoint = "false";
	      graph.options().setOptionsFromURL(defaultConfig, false);
	    }
